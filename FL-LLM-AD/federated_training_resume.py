...
# Garantir que a pasta de emissões existe
emissions_dir = f"./fl-results/{args.sim_name}/emissions"
...
# Detectar automaticamente o último round existente
fl_path = f"./fl-results/{SIM_NAME}"
os.makedirs(fl_path, exist_ok=True)
existing_rounds = [int(d.split('_')[1]) for d in os.listdir(fl_path) if d.startswith('round_') and d.split('_')[1].isdigit()]
start_round = max(existing_rounds) + 1 if existing_rounds else 1

print(f"⚙️  Detected existing rounds: {existing_rounds}")
print(f"🔁  Resuming from round {start_round} até {NUM_ROUNDS}")
...
for round in range(start_round, NUM_ROUNDS + 1):
    ...
    for client in clients:
        ...
        client_model = train_client(
            ...
            max_steps=max_steps, lr=new_lr, batch_size=batch_size,
            model_name=MODEL_NAME, lora=lora
        )
